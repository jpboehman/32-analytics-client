...
export const NCAATeamEPSSPage = () => {
  ...
  const Background = () => {
    return (
      ...
      >
        <Typography.Title level={1} style={{ color: '#fff', marginLeft: 20 }}>
          {fixedHeaderText}
        </Typography.Title>
        <Typography.Title style={{ color: '#fff', marginLeft: 20 }} level={5}>
          --------------------------------------------------------
        </Typography.Title>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {currentUser && (
        <div>{ncaaTeamEpss.length ? <Datatable6 /> : <SmallLoader />}</div>
      )}
      <ScrollToTop />
      <Footer />
    </div>
